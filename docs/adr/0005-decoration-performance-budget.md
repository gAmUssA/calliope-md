...
4. **Render only the viewport plus a 50-line buffer.** `filterByVisibleRange` drops every element outside `getVisibleRangeWithBuffer(editor)` before decorations are constructed.
